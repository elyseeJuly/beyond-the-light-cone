  ...
  const handleStartSmartDownload = async () => {
    ...
    for (let i = 0; i < pendingPacks.length; i++) {
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          assetLoader.downloadPack(packId, (p) => {
            if (p.state === 'downloading') {
              ...
              reject(new Error(p.error || t('未知下载错误')));
            }
          }).catch(reject);
        });
        done++;
        ...
        setErrorMessage(err.message || t('网络连接超时，下载中断。'));
        ...
        <div className="text-xs text-slate-300 leading-relaxed z-10 flex-grow min-h-0">
          {downloadingState === 'idle' && (
            <div className="space-y-4">
              <p>
                {t("文明执政官，系统自检已完成。游戏核心组件（Layer 1 Core）已完全加载就绪。")}
              </p>
              <p>
                {t("为优化首屏加载速率，高精度结局CG、角色全息立绘及宏大的纪元原声带等大体量多媒体资源（约")} <span className="text-cyan-400 font-bold">370MB</span>{t("）被归为")}<strong>{t("分段扩展数据包")}</strong>。
              </p>
              <p className="text-[11px] text-slate-400 italic">
                * {t("建议立刻授权全量下载，以保障在后续时代更替（如危机、威慑纪元等）时获得完整的视听沉浸体验。")}
              </p>
            </div>
          )}
          ...
          {downloadingState === 'downloading' && (
            <div className="space-y-5 py-4">
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-slate-400">{t("总数据包下载进度")}:</span>
                <span className="text-cyan-400 font-bold">{completedCount} / {totalPending} {t("已完成")}</span>
              </div>
          ...
          {downloadingState === 'complete' && (
            <div className="space-y-4 py-2">
              ...
              <p>
                {t("已将所有扩展多媒体数据包安全下载并缓存至您的银河数据库。接下来，您将体验到最完整的《光锥之外》声画特效。")}
              </p>
            </div>
          )}
          ...
          {downloadingState === 'error' && (
            <div className="space-y-4">
              ...
              <p>
                {t("当前连接信号受智子波动干扰。您可以使用“暂不下载”直接进入游戏，或尝试“重新连接”。")}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-3.5 shrink-0 z-10">
          {downloadingState === 'idle' && (
            <>
              ...
              >
                <Download size={14} /> {t("一键智能全量下载 (推荐)")}
              </button>
