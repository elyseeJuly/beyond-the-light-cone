...
export class Game {
  ...
  private handleSubsystemError(context: string, error: any): void {
    ...
    this.addHistory(t("[警告] {param0}: {param1}", { param0: context, param1: error?.message || error }));
  }
  ...
  public addHistory(log: string, overrideYear?: number, overrideEpoch?: EpochType): void {
    ...
    const prefix = t("{param0} {param1} 年 - ", { param0: epochNames[targetEpoch], param1: targetYear });
  ...
  public isSophonBlocked(): boolean {
    ...
    const sanTi = this.alienCiviManager.aliens.get(t("三体"));
    if (sanTi && !sanTi.isDieOut() && sanTi.friendshipType < FriendshipType.FRIEND) {
      const tecMgr = this.earthCivi.tecTreeManager;
      if (tecMgr.isTecFinished(TecTreeType.INFORMATION, t("550W量子计算机")) ||
          tecMgr.isTecFinished(TecTreeType.PHYSICS, t("智子工程"))) {
        return false;
      }
      return true;
    }
  ...
  public runAIBrain(): void {
    ...
    if (civi.isResearchIdle() && civi.canSpendAP(10)) {
      ...
      if (best) {
        ...
        actions.push(t("🤖 [AI智脑] 已自动将科研重心转移至『{param0}』", { param0: best.node }));
      }
    }
    ...
    if (civi.resource < 50 && civi.miningRatio < 60 && civi.canSpendAP(5)) {
      ...
      if (boost > 0) {
        ...
        actions.push(t("🤖 [AI智脑] 资源紧张，已自动将采矿比例提升至 {param0}%", { param0: civi.miningRatio }));
      }
    }
    ...
    if (civi.economy < 50 && civi.factoryRatio < 50 && civi.canSpendAP(5)) {
      ...
      if (boost > 0) {
        ...
        civi.spendAP(5);  // AI 模式自动半价 = 2
        civi.allocateWorkers();
        actions.push(t("🤖 [AI智脑] 经济低迷，已自动将工厂比例提升至 {param0}%", { param0: civi.factoryRatio }));
      }
    }
    ...
    if (hasEmptyDept && civi.canSpendAP(5)) {
      ...
      civi.spendAP(5);  // AI 模式自动半价 = 2
      actions.push(t("🤖 [AI智脑] 已自动补全部门首长空缺"));
    }
    ...
    if (this.currentEvent) {
      ...
      if (defaultChoice) {
        ...
        actions.push(t("🤖 [AI智脑] 已自动处理剧情事件「{param0}」", { param0: eventTitle }));
      }
    ...
    while (this.eventQueue.length > 0) {
      ...
      if (defaultChoice) {
        defaultChoice.action();
        actions.push(t("🤖 [AI智脑] 已自动处理剧情事件「{param0}」", { param0: ev.title }));
      }
    }
  ...
  public getTurnBlockers(): string[] {
    ...
    if (civi.resource <= 10) {
      blockers.push(t("资源崩盘：资源储备即将耗尽"));
    }
    if (civi.economy <= 10) {
      blockers.push(t("经济危机：经济产出濒临崩溃"));
    }
    // 宽限期内科研停滞不阻断，仅作为警告
    if (!grace && civi.isResearchIdle()) {
      blockers.push(t("科研停滞：未指派任何研究项目"));
    }
    // 宽限期内部门首长空缺不阻断，仅作为警告
    if (!grace) {
      let hasEmptyDept = false;
  ...
  public getTurnWarnings(): string[] {
    ...
    if (civi.isResearchIdle()) {
      warnings.push(t("科研停滞：未指派任何研究项目"));
    }
    let hasEmptyDept = false;
    for (const dept of civi.departments.values()) {
      if (!dept.leaderName) { hasEmptyDept = true; break; }
  ...
  public runARound(): void {
    ...
    if (this.currentEvent || this.eventQueue.length > 0) {
      if (this.earthCivi.isAiBrainEnabled) {
        ...
        this.addHistory(t("提示：请先处理当前的剧情事件。"));
        return;
      }
    }
    ...
    if (!this.earthCivi.isAiBrainEnabled && !isTutorialActive) {
      ...
      if (blockers.length > 0) {
        ...
        this.addHistory(t("⚠ 回合被阻断：存在需要手动处理的紧急事务。"));
        return;
      }
    }
    ...
    this.addHistory(t(">>> 开始结算当前回合逻辑..."));

    try {
      this.addHistory(t("...正在处理文明资源与人口增长"));
      try {
        this.earthCivi.runARound();
      } catch (e: any) {
        this.handleSubsystemError(t("地球模拟出现异常"), e);
      }

      this.addHistory(t("...正在推进发动机与数字生命结算"));
      try {
        this.planetEngine.processTurn();
        this.digitalLife.processTurn();
      } catch (e: any) {
        this.handleSubsystemError(t("推进引擎子系统异常"), e);
      }

      this.addHistory(t("...正在评估异星文明威胁"));
      try {
        this.alienCiviManager.runARound();
      } catch (e: any) {
        this.handleSubsystemError(t("异星模拟出现异常"), e);
      }
      if (this.earthCivi) {
        this.earthCivi.swordholderHandoverTurn = false;
      }

      this.addHistory(t("...正在更新外交冷却与通信信道"));
          ...
          action: () => {
            ...
            if (c.effects && c.effects.some((eff: any) => eff.target === FLAG.SWORDHOLDER_APPOINTED)) {
              ...
              if (luoji && luoji.isAlive) {
                this.earthCivi.swordholder = t("罗辑");
              } else {
      ...
      tickerEvents.forEach(e => {
        ...
        this.playerTimeline.push({
          year: this.year,
          event: t("重大发现：{param0} —— {param1}", { param0: e.name, param1: text })
        });
      ...
      try {
        ...
        if (this.earthCivi.population < 20 && !this.tagManager.hasTag('population_crisis')) {
          ...
          this.historyGenerator.recordTagChange(this.year, this.epoch, 'population_crisis', t("人口危机"), true);
        }
        if (this.earthCivi.treachery > 60 && !this.tagManager.hasTag('civil_unrest')) {
          ...
          this.historyGenerator.recordTagChange(this.year, this.epoch, 'civil_unrest', t("民心不稳"), true);
        }
        if (this.earthCivi.deterrenceValue > 60 && !this.tagManager.hasTag('deterrence_steady')) {
          ...
          this.historyGenerator.recordTagChange(this.year, this.epoch, 'deterrence_steady', t("威慑稳固"), true);
        }
      } catch (e: any) {
        this.handleSubsystemError(t("Tag 系统异常"), e);
      }
      ...
      try {
        ...
        if (this.atmosphereEngine.transitionTo(newAtmosphere) && prevAtmosphere !== newAtmosphere) {
          this.addHistory(t("【氛围变化】{param0}: {param1}", { param0: this.atmosphereEngine.getConfig().label, param1: this.atmosphereEngine.getConfig().description }));
          this.historyGenerator.recordEvent(this.year, this.epoch, t("氛围变化"), t("游戏氛围变为「{param0}」", { param0: this.atmosphereEngine.getConfig().label }));
        }
      } catch (e: any) {
      ...
      interactiveEvents.forEach(e => {
        this.addHistory(t("触发抉择事件: {param0}", { param0: e.name }));
        ...
        const choices = e.choices && e.choices.length > 0
          ? e.choices.map(c => ({
              ...
              action: () => {
                ...
                this.playerTimeline.push({
                  year: eventYear,
                  event: t("在「{param0}」事件中做出选择：{param1}", { param0: e.name, param1: c.label })
                });
                this.addHistory(t("[抉择结果] {param0} -> 选择了「{param1}」", { param0: e.name, param1: c.label }), eventYear, eventEpoch);
          ...
          : [{
              ...
              action: () => {
                ...
                this.playerTimeline.push({
                  year: eventYear,
                  event: t("确认了重大历史事件「{param0}」", { param0: e.name })
                });
      ...
      if (interactiveEvents.length === 0) {
        try {
          ...
          for (const alien of this.alienCiviManager.aliens.values()) {
            if (alien.discovered && !alien.isDieOut()) {
              const rel = this.relationNetwork.getRelation(t("地球"), alien.name);
              if (rel) {
          ...
          this.handleSubsystemError(t("关系网络更新异常"), e);
        }

        try {
          const slice = this.sliceNarrativeEngine.generateSlice(
            `auto_turn_${this.year}`, t("年份推进"), this.tagManager, this.year
          );
          if (slice) {
            const msg = `${slice.characterName}(${slice.characterRole}): ${slice.innerMonologue}`;
            this.tickerMessages.push(msg);
            this.addHistory(t("【叙事片段】{param0}", { param0: msg }));
            this.eventBus.emitLegacy('ticker-message-added');
          }
        } catch (e: any) {
          this.handleSubsystemError(t("叙事片段生成异常"), e);
        }
        ...
        if (this.year === 50 && !this.flagManager.isSet(FLAG.RUINS_CHECKED)) {
          ...
          if (ruins.length > 0) {
            ...
            const ruinsEvent: GameEventPayload = {
              ...
              dialogQueue: [{
                speakerName: t("科学执政官"),
                content: t("深空探测器在远古坐标发现了一处破碎的人类飞船遗迹。这似乎是另一个平行时间线中，毁灭于 {param0} 年、曾积累了 {param1} 文化强度的古老地球文明残留物。我们的科学家从残骸中解译出了一些技术图纸。", { param0: latestRuin.year, param1: latestRuin.culture })
              }],
              choices: [{
        ...
        this.addHistory(t("回合推进完成：{param0} -> {param1} (存活异星文明: {param2}, 待处理事件: {param3})", { param0: this.year - 1, param1: this.year, param2: this.alienCiviManager.aliens.size, param3: this.eventQueue.length }));
        this.eventBus.emitLegacy('game-turn-complete');
      } else {
        this.processNextEvent();
        this.addHistory(t("已触发交互事件，年份推进暂缓 (存活异星文明: {param0}, 待处理事件: {param1})", { param0: this.alienCiviManager.aliens.size, param1: this.eventQueue.length }));
      }
    } catch (err: any) {
      this._hadRunError = true;
      if (Game.strictMode) {
        throw err instanceof Error ? err : new Error(t("核心结算失败: {param0}", { param0: err?.message || t("未知错误") }));
      }
      console.error("Critical error in runARound:", err);
      this.addHistory(t("【核心崩溃】结算失败! 错误详情: {param0}", { param0: err?.message || t("未知错误") }));
      this.addHistory(t("系统已尝试紧急回滚状态锁，请尝试再次点击或重新开始。"));
    } finally {
      ...
      if (!p.isAlive) {
        ...
        if (p.deathYear === 0 || p.deathYear === this.year) {
          ...
          if (appeared.has(p.name)) {
            this.addHistory(t("【讣告】{param0} 结束了波澜壮阔的一生，于 {param1} 年逝世。", { param0: p.name, param1: this.year }));
            this.tickerMessages.push(t("讣告：{param0} 逝世。", { param0: p.name }));
          }
        }
      }
    }
  ...
  public updateEpoch(): void {
    ...
    if (prevEpoch !== this.epoch) {
      ...
      this.addHistory(t("【纪元更替】进入{param0}！", { param0: epochName }));
      this.playerTimeline.push({ year: this.year, event: t("【纪元更替】人类正式进入{param0}", { param0: epochName }) });
      ...
      if (currentEraKey) {
        assetLoader.downloadEraPack(currentEraKey).catch(err => {
          console.warn(t("[Game] 纪元资源包下载失败 ({param0}):", { param0: currentEraKey }), err);
        });
        assetLoader.preloadNextEra(currentEraKey).catch(err => {
          console.warn(t("[Game] 下一纪元预加载失败 ({param0}):", { param0: currentEraKey }), err);
        });
      }

      // 时间线锚点：从 timeline.json 注入底部资讯滚动播报
      const timelineEntry = timelineData.find(entry => entry.epoch === epochName || 
        (epochName === t("银河纪元") && entry.epoch.startsWith(t("银河纪元"))) ||
        (epochName === t("星屑纪元") && entry.epoch.includes(t("星屑"))));
      if (timelineEntry) {
      ...
      const epochContents: Record<number, string> = {
        0: t("那是上个世纪的往事。人类尚未意识到宇宙的险恶，在懵懂中向星空发出了第一声呼唤，黄金岁月还在继续。"),
        1: t("人类发现了三体舰队，全世界进入危机纪元。行星防御理事会正式启动面壁计划，基础物理已被智子封锁，人类必须寻找在围剿下存活的手段！"),
        2: t("威慑平衡正式建立，人类世界进入威慑纪元。在执剑人的威慑威压下，三体文明被迫停止了向太阳系的扩张，进入脆弱而短暂的和平冷战期。"),
        3: t("威慑宣告中止，万有引力号启动了坐标广播，人类正式步入广播纪元。两个世界的坐标已暴露在黑暗森林法则的枪口之下，毁灭倒计时开始。"),
        4: t("太阳系黑暗森林打击临近，掩体世界群宣告落成，人类迈进掩体纪元。数十座宏伟太空城散布在气态行星背面，人类试图借此躲过光粒打击。"),
        5: t("太阳系终遭降维打击，大批光速飞船破空而去，逃亡银河系，开启银河纪元。地球不再是人类唯一的家园，人类火种从此散布在浩瀚星海。"),
        6: t("大宇宙的结构在战争中进一步降维碎裂。太阳系乃至银河系的核心都已退化崩缩，人类分散在各个漂浮的碎星和微型星云间挣扎求生。这是一个万物归尘、同时也是最后的星屑纪元。")
      };
      ...
      const newEpochEvent: GameEventPayload = {
        ...
        title: t("纪元更替：{param0}", { param0: newEpochName }),
        dialogQueue: [{
          speakerName: t("历史观测记录"),
    ...
    return [
      {
        type: "HIDDEN",
        label: t("死神永生 · 小宇宙"),
        description: t("归零者的讯息抵达，人类选择将小宇宙的质量归还大宇宙，文明化为永恒的生态球"),
        ...
        check: () => {
          ...
          return tm.isTecFinishedAnywhere(t("黑域生成")) && tm.isTecFinishedAnywhere(t("数字方舟"));
        },
        progress: () => {
          let p = 0;
          ...
          const tm = this.earthCivi.tecTreeManager;
          if (tm.isTecFinishedAnywhere(t("黑域生成"))) p += 3;
          if (tm.isTecFinishedAnywhere(t("数字方舟"))) p += 2;
      ...
      },
      {
        type: "WANDERING",
        label: t("流浪胜利"),
        description: t("完成行星发动机Ⅲ型与新家园选址，带领地球踏上星辰大海"),
        allowedEras: [EpochType.BUNKER, EpochType.GALAXY, EpochType.STARDUST],
        check: () => {
          const tm = this.earthCivi.tecTreeManager;
          return this.year >= 250 &&
                 this.earthCivi.population > 0 &&
                 tm.isTecFinished(TecTreeType.AEROSPACE, t("行星发动机Ⅲ型")) &&
                 tm.isTecFinished(TecTreeType.INTERSTELLAR, t("新家园选址")) &&
        ...
        progress: () => {
          let p = 0;
          const tm = this.earthCivi.tecTreeManager;
          if (tm.isTecFinished(TecTreeType.AEROSPACE, t("行星发动机Ⅲ型"))) p += 25;
          if (tm.isTecFinished(TecTreeType.INTERSTELLAR, t("新家园选址"))) p += 25;
      ...
      },
      {
        type: "DIGITAL",
        label: t("数字永生胜利"),
        description: t("完成数字方舟，将人类意识上传至虚拟世界"),
        allowedEras: [EpochType.BUNKER, EpochType.GALAXY, EpochType.STARDUST],
        check: () => {
          return this.year >= 200 &&
                 this.earthCivi.population > 50 &&
                 this.earthCivi.tecTreeManager.isTecFinished(TecTreeType.INFORMATION, t("数字方舟")) &&
                 this.hasFlag(FLAG.DIGITAL_ARK_UPGRADE) &&
                 !this.hasFlag(FLAG.WANDERING_COMPLETED) &&
                 !this.hasFlag(FLAG.DARK_DOMAIN_DECISION) &&
                 !this.hasFlag(FLAG.CONQUEST_DECLARED) &&
        ...
        progress: () => {
          let p = 0;
          if (this.earthCivi.tecTreeManager.isTecFinished(TecTreeType.INFORMATION, t("数字方舟"))) p += 40;
      ...
      },
      {
        type: "DETERRENCE",
        label: t("威慑胜利"),
        description: t("在威慑纪元中拥有执剑人，维持威慑平衡"),
      ...
      },
      {
        type: "DARK_DOMAIN",
        label: t("黑域胜利"),
        description: t("完成黑域生成技术，发布宇宙安全声明"),
        allowedEras: [EpochType.BUNKER, EpochType.GALAXY, EpochType.STARDUST],
        check: () => {
          return this.year >= 250 &&
                 this.earthCivi.population > 0 &&
                 this.earthCivi.tecTreeManager.isTecFinishedAnywhere(t("黑域生成")) &&
                 this.hasFlag(FLAG.DARK_DOMAIN_DECISION) &&
        ...
        progress: () => {
          let p = 0;
          if (this.earthCivi.tecTreeManager.isTecFinishedAnywhere(t("黑域生成"))) p += 40;
  ...
  public checkVictoryConditions(): void {
    ...
    if (this.broadcastTriggered) {
      ...
      if (this.broadcastSurvives) {
        ...
        this.gameOverReason = t("太阳系坐标宣告暴露，但幸存的人类先驱已通过光速飞船或数字方舟逃逸。在大宇宙热寂到来之前，你们在归零者的小宇宙中将火种延续下去。");
        ...
        this.gameOverReason = t("引力波发射塔发射了精确的星系坐标信号，黑暗森林打击全面爆发。地球和三体世界在光粒打击中双双被湮灭，未做逃逸准备的人类文明彻底断绝。");
        SaveManager.recordEnding({
          victoryType: null,
    ...
    for (const cond of conditions) {
      ...
      if (cond.check()) {
        ...
        this.playerTimeline.push({ year: this.year, event: t("【大结局】达成 {param0}", { param0: cond.label }) });
    ...
    if (this.epoch >= EpochType.GALAXY && this.hasFlag(FLAG.GALAXY_EXODUS_SEEN) &&
      ...
      this.gameOverReason = t("永恒的流亡：地球已被遗弃，幸存的人类乘坐星舰在黑暗的宇宙中无尽漂流，成为永远的星际游牧民族。");
      this.playerTimeline.push({ year: this.year, event: t("【终结】人类文明化为永恒的星舰流亡者") });
      SaveManager.recordEnding({
    ...
    if (this.epoch >= EpochType.BUNKER &&
      ...
      this.gameOverReason = t("宇宙静默：战争与扩张失去了意义，文明选择了向内探索。我们不再发出任何声音，彻底融入了宇宙的背景辐射之中。");
      this.playerTimeline.push({ year: this.year, event: t("【终结】人类文明选择了永恒的静默") });
      SaveManager.recordEnding({
        victoryType: null, defeatType: null, neutralType: this.neutralType,
        label: t("宇宙静默"),
        year: this.year, epoch: this.epoch, keyFlags: this.flagManager.getSnapshot(), timestamp: Date.now()
    ...
    if (this.earthCivi.treachery >= 100) {
      ...
      this.gameOverReason = t("逃亡主义失控：人类放弃了最后的希望，文明在内耗中走向崩溃。");
      this.playerTimeline.push({ year: this.year, event: t("【终结】逃亡主义吞噬了文明最后的秩序") });
      SaveManager.recordEnding({
        victoryType: null, defeatType: this.defeatType, label: t("逃亡主义崩溃"),
        year: this.year, epoch: this.epoch, keyFlags: this.flagManager.getSnapshot(), timestamp: Date.now()
      });
      // 结局前自动存档
      SaveManager.autoSave(() => JSON.stringify(this, gameReplacer));
    ...
    if (this.earthCivi.population <= 0) {
      this.isGameOver = true;
      this.defeatType = DefeatType.EXTINCTION;
      this.gameOverReason = t("文明灭绝：地球已成为一颗死寂的星球。");
      this.playerTimeline.push({ year: this.year, event: t("【终结】最后的人类在沉默中消逝") });
      SaveManager.recordEnding({
        victoryType: null, defeatType: this.defeatType, label: t("文明灭绝"),
        year: this.year, epoch: this.epoch, keyFlags: this.flagManager.getSnapshot(), timestamp: Date.now()
      });
    ...
    if ((this.year > 350 || this.dimensionStrikeTriggered) &&
        !this.earthCivi.tecTreeManager.isTecFinishedAnywhere(t("黑域生成")) &&
        !this.earthCivi.tecTreeManager.isTecFinishedAnywhere(t("数字方舟")) &&
      ...
      if (this.dimensionStrikeTriggered) {
        ...
        this.gameOverReason = t("二向箔打击：黑暗森林打击降临，太阳系从三维空间跌入二维。文明未能逃逸。");
        this.playerTimeline.push({ year: this.year, event: t("【终结】二向箔降维打击抹去了整个太阳系") });
        ...
        this.gameOverReason = t("太阳氦闪：漫长的等待终结于刺眼的白光，地球未能逃离。");
        this.playerTimeline.push({ year: this.year, event: t("【终结】太阳的死亡终结了一切") });
      }
      SaveManager.recordEnding({
        victoryType: null, defeatType: this.defeatType, label: this.dimensionStrikeTriggered || this.loreMode === 'strict_three_body' ? t("二向箔打击") : t("太阳氦闪"),
        year: this.year, epoch: this.epoch, keyFlags: this.flagManager.getSnapshot(), timestamp: Date.now()
      });
  ...
  public conductDiplomacy(alienName: string, actionType: string): string {
    ...
    if (!result.startsWith(t("无法")) && !result.startsWith(t("经济不足")) && !result.includes(t("失败"))) {
      ...
      if (alien) {
        ...
        if (relDelta !== 0) {
          this.relationNetwork.modifyRelation(t("地球"), alienName, relDelta);
        ...
        if (alien.friendshipType >= FriendshipType.FRIEND && actionType === 'alliance') {
          ...
          this.tickerMessages.push(t("【星际外交】人类与 {param0} 正式缔结同盟条约，开启星际合作新纪元！", { param0: alienName }));
          this.eventBus.emitLegacy('ticker-message-added');
        }
  ...
  private _conductDiplomacyInternal(alienName: string, actionType: string): string {
    const alien = this.alienCiviManager.aliens.get(alienName);
    if (!alien || alien.isDieOut()) return t("无法与已灭亡的文明 {param0} 进行外交。", { param0: alienName });
    if (!alien.contacted) return t("【通信未建立】人类尚未与 {param0} 建立可外交的通信信道，无法执行外交行动。", { param0: alienName });
    if (alien.diplomacyCooldown > 0) return t("外交冷却中，还需等待 {param0} 回合。", { param0: alien.diplomacyCooldown });
    ...
    if (alienName === t("三体")) {
      switch (actionType) {
        case 'negotiate':
          ...
          return t("与 {param0} 进行外交和平谈判。关系得到改善，但因释放和平信号，对三体威慑度下降 10%（当前威慑度: {param1}%）。", { param0: alienName, param1: Math.floor(e.deterrenceValue) });
        case 'trade':
          if (e.economy >= 30) {
            e.economy -= 30;
            e.resource += 50;
            e.deterrenceValue = Math.max(0, e.deterrenceValue - 15);
            return t("与 {param0} 进行了“三体文化与科技交流”：-30 经济，+50 资源。人类社会沉浸在三体文化的温吞中，威慑度下降 15%（当前威慑度: {param1}%）。", { param0: alienName, param1: Math.floor(e.deterrenceValue) });
          }
          return t("经济不足以进行贸易（需要30）。");
        case 'provoke':
          if (!e.swordholder) {
            return t("【威慑失败】当前没有执剑人在位，无法发起坐标广播威慑。威胁被判定为虚张声势！");
          }
          alien.friendshipType = FriendshipType.VERYANGRY;
          e.deterrenceValue = Math.min(100, e.deterrenceValue + 20);
          return t("【威慑提升】人类通过执剑人 {param0} 对 {param1} 进行了引力波广播威慑威胁！威慑度提升 20%，当前威慑度: {param2}%，关系恶化至极度敌对。", { param0: e.swordholder, param1: alienName, param2: Math.floor(e.deterrenceValue) });
        case 'alliance':
          if (e.deterrenceValue >= 90) {
            alien.isBund = true;
            alien.friendshipType = Math.min(FriendshipType.VERYFRIEND, alien.friendshipType + 1);
            return t("【战略同盟】在高达 {param0}% 的绝对威慑力下，{param1} 被迫妥协，与人类签署了《太阳系-三体威慑平衡和平同盟协定》！", { param0: Math.floor(e.deterrenceValue), param1: alienName });
          }
          return t("【结盟失败】{param0} 拒绝了和平结盟。三体文明回复：“我们在人类的执剑人身上看到了软弱与迟疑。威慑度不足以保障我们的平等共存。”", { param0: alienName });
        default:
          return t("未知的外交行动：{param0}", { param0: actionType });
      }
    }
    ...
    switch (actionType) {
      case 'negotiate':
        alien.friendshipType = Math.min(FriendshipType.VERYFRIEND, alien.friendshipType + 1);
        return t("与 {param0} 的外交谈判取得进展，关系提升。", { param0: alienName });
      case 'trade':
        if (e.economy >= 30) {
          e.economy -= 30;
          e.resource += 50;
          return t("与 {param0} 完成贸易交换：-30经济，+50资源。", { param0: alienName });
        }
        return t("经济不足以进行贸易（需要30）。");
      case 'provoke':
        alien.friendshipType = Math.max(FriendshipType.VERYANGRY, alien.friendshipType - 1);
        return t("对 {param0} 发出战争挑衅，关系恶化。", { param0: alienName });
      case 'alliance':
        if (alien.friendshipType >= FriendshipType.FRIEND) {
          alien.isBund = true;
          alien.friendshipType = Math.min(FriendshipType.VERYFRIEND, alien.friendshipType + 1);
          return t("与 {param0} 正式结成战略同盟！", { param0: alienName });
        }
        return t("{param0} 拒绝了同盟提议，关系不足。", { param0: alienName });
      default:
        return t("未知的外交行动：{param0}", { param0: actionType });
    }
  }
  ...
  public updateDiplomacyUnlocks(): void {
    const trisolaris = this.alienCiviManager.aliens.get(t("三体"));
    if (trisolaris) {
    ...
    const tryDiscover = (alien: AlienCivilization | undefined, condition: boolean, discovery: { title: string; tip: string; speaker: string; content: string; avatar?: string }) => {
      ...
      if (condition && !alien.discovered) {
        ...
        const msg = t("【首次发现】人类观测到异星文明「{param0}」的存在信号！", { param0: alien.name });
        this.addHistory(msg);
    ...
    const tryContact = (alien: AlienCivilization | undefined, condition: boolean, contact: { title: string; tip: string; speaker: string; content: string; avatar?: string }) => {
      ...
      if (condition && alien.discovered && !alien.contacted) {
        ...
        const msg = t("【探索信道解锁】成功建立与异星文明「{param0}」的通信信道！", { param0: alien.name });
        this.addHistory(msg);
        this.pushTickerMessage(msg);
        this.dispatchTickerEvent();
    ...
    // 歌者：远距离观测信号 → 可外交
    const singer = this.alienCiviManager.aliens.get(t("歌者"));
    tryDiscover(singer, this.year >= 120 || hasTech(t("太阳波放大器50光年")), {
      title: t("深空光粒信号"),
      tip: t("深空观测站捕获到一段异常的高频光粒波段，其模式不可能是自然现象。某种智慧文明正在清理宇宙。"),
      speaker: t("深空观测站"),
      content: t("长官，我们在太阳系边缘捕捉到了一段高频光粒波段。这不是自然背景噪声——它太规律了，像是某种飞船或武器留下的痕迹。")
    });
    tryContact(singer, hasTech(t("1万光年远镜")) || hasTech(t("太阳波放大器50光年")) || this.year >= 150 || this.hasFlag(FLAG.SINGER_CONTACT), {
      title: t("歌者文明接触"),
      tip: t("通过太阳波放大器，人类终于与那个在黑暗中清理宇宙的文明建立了脆弱的通信信道。"),
      speaker: t("通讯解码员"),
      content: t("我们成功解码了信号。对方自称‘歌者’，对他们来说，毁灭一个文明就像捡起一张废纸一样平常。")
    });

    // 魔戒：四维遗迹发现 → 可外交
    const ring = this.alienCiviManager.aliens.get(t("魔戒"));
    tryDiscover(ring, hasTech(t("宇宙社会学")) || this.earthCivi.starIndices.has(10) || this.earthCivi.starIndices.has(11), {
      title: t("四维空间遗迹"),
      tip: t("探索飞船在太阳系边缘发现了一个不该存在的四维空间碎块，其中似乎封存着某种古老的生命信号。"),
      speaker: t("探索队队长"),
      content: t("长官，我们找到了一个四维碎块。三维空间中不该有这样的东西存在……里面有什么东西在回应我们的探测。")
    });
    tryContact(ring, hasTech(t("10%光速飞船")) || this.earthCivi.starIndices.has(10) || this.earthCivi.starIndices.has(11) || this.hasFlag(FLAG.RING_CONTACT), {
      title: t("魔戒文明接触"),
      tip: t("探索队成功与四维碎块中的生命体建立了通信。它们自称‘墓地’，是来自更高维度的遗民。"),
      speaker: t("魔戒"),
      content: t("海？……水？……你们是低维世界的新生命。我们只剩下记忆，但愿意与你们交谈。")
    });

    // 边缘世界：引力波信号 → 可外交
    const fringe = this.alienCiviManager.aliens.get(t("边缘世界"));
    tryDiscover(fringe, this.epoch >= EpochType.BROADCAST || hasTech(t("引力波广播系统")), {
      title: t("遥远战场的回声"),
      tip: t("引力波天线捕捉到一场正在进行的星际战争信号。交战的一方，似乎与三体文明有着深仇大恨。"),
      speaker: t("引力波监听员"),
      content: t("我们捕捉到了持续不断的引力波涟漪。有人在和三体舰队交战，而且他们离太阳系并不算太远。")
    });
    tryContact(fringe, hasTech(t("99%光速飞船")) || hasTech(t("引力波广播系统")) || this.epoch >= EpochType.BROADCAST || this.hasFlag(FLAG.FRINGE_CONTACT), {
      title: t("边缘世界接触"),
      tip: t("通过引力波广播系统，人类与正在和三体文明交战的‘边缘世界’建立了联系。"),
      speaker: t("边缘世界使者"),
      content: t("三体不是你们唯一的敌人，也不是最强的。我们可以是盟友，如果你们愿意共享情报。")
    });

    // 归零者：超维广播 → 可外交（神级文明）
    const zeroers = this.alienCiviManager.aliens.get(t("归零者"));
    tryDiscover(zeroers, hasTech(t("归零者研究")) || this.year >= 260, {
      title: t("全宇宙广播"),
      tip: t("一股来自宇宙深处的超维广播穿透了所有物理屏障。这不是任何已知文明能做到的事。"),
      speaker: t("宇宙学研究院"),
      content: t("这段广播同时出现在所有频段、所有维度上。它来自……宇宙之外，或者宇宙之始。")
    });
    tryContact(zeroers, hasTech(t("归零者研究")) || this.hasFlag(FLAG.ZEROERS_CONTACT) || this.year >= 280, {
      title: t("归零者接触"),
      tip: t("人类终于回应了归零者的召唤。这个神级文明想要重启宇宙，而人类有机会参与其中。"),
      speaker: t("归零者"),
      content: t("归还你们小宇宙中的质量。宇宙需要重启，否则一切将在热寂中终结。")
    });

    // 碳基联邦 / 硅基帝国：银河系远镜 → 可外交
    const carbon = this.alienCiviManager.aliens.get(t("碳基联邦"));
    tryDiscover(carbon, hasTech(t("银河系远镜")) && this.year >= 120, {
      title: t("银河遗迹信号"),
      tip: t("银河系远镜捕捉到了一场远古战争的遗迹。那是碳基生命与硅基生命曾经争霸银河的证据。"),
      speaker: t("银河系远镜观测员"),
      content: t("我们在银河系旋臂发现了大量有机物战舰残骸。这里发生过一场史诗级战争。")
    });
    tryContact(carbon, hasTech(t("银河系远镜")) && this.year >= 150, {
      title: t("碳基联邦接触"),
      tip: t("银河系远镜成功定位了碳基联邦的残存舰队。它们曾是银河系的守护者之一。"),
      speaker: t("碳基联邦外交官"),
      content: t("硅基帝国并未被彻底消灭。如果你们不想重蹈我们的覆辙，就加入我们。")
    });

    const silicon = this.alienCiviManager.aliens.get(t("硅基帝国"));
    tryDiscover(silicon, hasTech(t("银河系远镜")) && this.year >= 120, {
      title: t("无机计算矩阵"),
      tip: t("银河系远镜捕捉到了高强度无机计算矩阵波动。某种非碳基文明仍在银河深处运转。"),
      speaker: t("计算矩阵分析员"),
      content: t("这些波动来自自我复制的计算节点。它们没有生物形态，但显然是智慧文明。")
    });
    tryContact(silicon, hasTech(t("银河系远镜")) && this.year >= 150, {
      title: t("硅基帝国接触"),
      tip: t("人类与硅基帝国建立了通信。它们对有机生命没有敌意，但也没有感情。"),
      speaker: t("硅基帝国节点"),
      content: t("碳基与硅基的战争已经结束。我们愿意与任何计算效率足够的文明进行数据交换。")
    });

    // 上帝文明 / 量子态文明：银河纪元 → 可外交
    const god = this.alienCiviManager.aliens.get(t("上帝文明"));
    tryDiscover(god, this.epoch >= EpochType.GALAXY && this.year >= 230, {
      title: t("衰亡的神级文明"),
      tip: t("深空舰队在银河系边缘遇到了一个正在衰亡的古老文明。它们自称‘上帝文明’，曾经创造过无数生命。"),
      speaker: t("深空舰队指挥官"),
      content: t("它们的飞船巨大而破败，但它们的技术仍然远超我们。它们说自己正在‘退休’。")
    });
    tryContact(god, this.epoch >= EpochType.GALAXY && this.year >= 250, {
      title: t("上帝文明接触"),
      tip: t("人类与上帝文明建立了通信。它们愿意分享自己最后的知识，但警告人类不要重蹈它们的覆辙。"),
      speaker: t("上帝文明"),
      content: t("我们曾经试图管理宇宙，但失败了。年轻人，不要犯我们犯过的错误。")
    });

    const quantum = this.alienCiviManager.aliens.get(t("量子态文明"));
    tryDiscover(quantum, this.epoch >= EpochType.GALAXY && this.year >= 230, {
      title: t("宏观量子涨落"),
      tip: t("物理学家观测到了呈现文明特征的宏观量子涨落。某种生命以量子态存在。"),
      speaker: t("量子物理学家"),
      content: t("这些涨落不是随机的。它们在‘思考’，以概率云的形式进行交流。")
    });
    tryContact(quantum, this.epoch >= EpochType.GALAXY && this.year >= 250, {
      title: t("量子态文明接触"),
      tip: t("人类首次与一个以量子叠加态存在的文明建立了通信。它们的生命形式完全超出传统生物学。"),
      speaker: t("量子态文明"),
      content: t("我们同时存在于许多状态。与你们交流，只是我们选择坍缩到这一个现实。")
    });
  }
  ...
  private enqueueAlienEvent(
    ...
    const payload: GameEventPayload = {
      ...
      choices: [{
        label: t("确认"),
        action: () => {
          this.addHistory(t("[{param0}] {param1} - 已与 {param2} 建立记录", { param0: kind === "discovery" ? t("首次发现") : t("通信建立"), param1: data.title, param2: alien.name }), this.year);
          this.applyEventEffect(EventEffect.NONE);
        }
      }]
    };
  ...
  public getEndingForecast(): Array<{ name: string; progress: number; isThreat: boolean }> {
    ...
    forecast.push({ name: t("氦闪危机"), progress: heliumProgress, isThreat: true });

    // 7. TREACHERY (Threat)
    const treacheryProgress = Math.floor(this.earthCivi.treachery);
    forecast.push({ name: t("逃亡崩溃"), progress: treacheryProgress, isThreat: true });
