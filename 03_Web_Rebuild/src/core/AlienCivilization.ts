...
export class AlienCivilization extends Civilization {
  ...
  public runARound(): void {
    ...
    if (this.name === t("三体") && game.earthCivi.swordholderHandoverTurn) {
    ...
    if (this.friendshipType >= FriendshipType.FRIEND && this.rng() < 0.1) {
      ...
      if (activeNodes.length > 0) {
        ...
        game.addHistory(t("【外交回馈】{param0} 与人类进行了技术交流，显著推进了「{param1}」的研发进度！", { param0: this.name, param1: target.name }));
      }
    }
  ...
  public launchHandoverWaterdropAttack(game: any, shName: string): void {
    ...
    const msg = t("【交接危机】智子判定新任执剑人「{param0}」威慑度不足，三体「水滴」探测器发起饱和打击！", { param0: shName });
    game.addHistory(msg);
    ...
    const targetIdx = 3;
    const fleet = createFleet(t("「水滴」交接突袭探测器"), this.name, targetIdx, 0, 3, true);
  ...
  private hunterBehavior(game: any, deterrenceRate: number): void {
    if (!game.earthCivi.isDieOut() && deterrenceRate < 90 && this.rng() < diplomacyData.hunterAttackChance) {
      ...
      if (this.attackCooldown === 0) {
        ...
        game.addHistory(t("【情报】{param0} 舰队正在集结，预计 {param1} 年后抵达太阳系。", { param0: this.name, param1: this.attackCooldown }));
      } else if (this.attackCooldown > 1) {
        this.attackCooldown--;
        game.addHistory(t("【情报】{param0} 舰队距抵达还有 {param1} 年。", { param0: this.name, param1: this.attackCooldown }));
      } else {
  ...
  private cleanerBehavior(game: any, deterrenceRate: number): void {
    if (!game.earthCivi.isDieOut() && deterrenceRate < 70 && this.rng() < diplomacyData.cleanerAttackChance) {
      if (this.attackCooldown === 0) {
        ...
        game.addHistory(t("【侦测】探测到 {param0} 具有清理倾向！舰队正在接近。", { param0: this.name }));
      } else if (this.attackCooldown > 1) {
        this.attackCooldown--;
      } else {
        this.attackCooldown = 0;
  ...
  private expansionistBehavior(game: any, deterrenceRate: number): void {
    if (this.rng() < 0.10) {
      ...
      if (unowned.length > 0 && deterrenceRate < 80) {
        ...
        game.addHistory(t("{param0} 扩张至 {param1}。", { param0: this.name, param1: target.name }));
      }
    }
  }
  ...
  private defensiveBehavior(game: any, _deterrenceRate: number): void {
    if (this.rng() < 0.05) {
      game.addHistory(t("{param0} 保持防御态势，加固现有领地。", { param0: this.name }));
    }
    this.army += 5;
  }
  ...
  private opportunistBehavior(game: any, deterrenceRate: number): void {
    if (this.friendshipType >= FriendshipType.FRIEND && this.rng() < diplomacyData.opportunistChance) {
      ...
      game.addHistory(t("{param0} 以友好名义索取了 {param1} 经济援助。", { param0: this.name, param1: received }));
      return;
    }
  ...
  private launchFleetAttack(game: any, eta: number): void {
    const targetIdx = 3;
    const fleet = createFleet(t("{param0} 远征军", { param0: this.name }), this.name, targetIdx, 0, eta);
    fleet.weapons.push({ weaponName: t("水滴型战舰"), currentBuild: 80 });
    fleet.weapons.push({ weaponName: t("强互作用探测器"), currentBuild: 40 });
    this.fleets.push(fleet);
    game.addHistory(t("【警报】{param0} 远征舰队已启程，预计 {param1} 年后抵达！", { param0: this.name, param1: eta }));
  }
  ...
  private processFleets(game: any): void {
    for (let i = this.fleets.length - 1; i >= 0; i--) {
      ...
      if (fleet.eta > 0) {
        fleet.eta--;
        if (fleet.eta === 0) {
          game.addHistory(t("【警报】{param0} 抵达太阳系！", { param0: fleet.name }));
          const earthTarget = game.starManager.getStar(fleet.targetStarIndex);
          if (earthTarget && earthTarget.belongToCivi === t("地球")) {
            ...
            if (win) {
              ...
              game.addHistory(t("【战败】地球被 {param0} 占领，人类文明遭受重创！", { param0: this.name }));
            } else {
              game.addHistory(t("【奇迹】地球守军成功击退了 {param0}！", { param0: fleet.name }));
  ...
  public launchWaterdropAttack(game: any): void {
    ...
    game.addHistory(t("【黑暗森林警报】智子观测到 {param0} 文明的「水滴」探测器已进入超高速巡航，预计 3 回合后抵达太阳系！", { param0: this.name }));
    
    const targetIdx = 3;
    const fleet = createFleet(t("「水滴」绝对静止探测器"), this.name, targetIdx, 0, 3, true);
    this.fleets.push(fleet);
  }
  ...
  public triggerDimensionStrike(game: any): void {
    ...
    game.addHistory(t("【死神警报】深空雷达侦测到一颗由 {param0} 抛出的微小薄膜物体正以光速扑向太阳系！高维空间崩塌倒计时开始：6回合后！", { param0: this.name }));
  }
  ...
  private processDimensionStrike(game: any): void {
    if (this.dimensionStrikeWarningTurns > 0) {
      ...
      if (this.dimensionStrikeWarningTurns > 0) {
        game.addHistory(t("【二向箔倒计时】坍缩波前锋正在加速靠近，剩余 {param0} 回合！", { param0: this.dimensionStrikeWarningTurns }));
      } else {
        game.addHistory(t("【高维坍缩】二向箔最终降临太阳系！三维空间以光速坍塌至二维！"));
        const tm = game.earthCivi.tecTreeManager;
        const survives = tm.isTecFinishedAnywhere(t("黑域生成")) || tm.isTecFinishedAnywhere(t("数字方舟")) || game.hasFlag(FLAG.GALAXY_EXODUS_SEEN) || game.hasFlag(FLAG.WANDERING_COMPLETED);
        
        if (survives) {
          game.addHistory(t("【生存奇迹】由于已构建光速安全声明/数字意识备份，人类文明的部分火种在坍缩中逃逸生存！"));
  ...
  public checkGravityBroadcast(game: any): void {
    if (!game.hasFlag(`${this.name}_broadcast_sent`)) {
      ...
      game.addHistory(t("【黑暗森林广播】{param0} 在母星系面临崩溃前，通过引力波向全宇宙广播了太阳系与三体星系的精确坐标！坐标已完全暴露！", { param0: this.name }));
  ...
  public loseStar(civiName: string, starIndex: number): void {
    ...
    if (alien) {
      ...
      if (alien.isDieOut() && this.#game) {
        this.#game.addHistory(t("【胜利】外星文明 {param0} 被彻底消灭！", { param0: civiName }));
      }
    }
  }
