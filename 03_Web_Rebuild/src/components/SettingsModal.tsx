...
export const SettingsModal: React.FC<SettingsModalProps> = ({ onClose, initialTab }) => {
    ...
    try {
      await assetLoader.downloadPack(packId, (p) => {
        ...
        if (p.state === 'complete') {
          ...
          alert(t("下载失败: {param0}", { param0: p.error || t('未知错误') }));
          setDownloadingPack(null);
        }
      });
    } catch (err) {
      alert(t("启动下载失败: {param0}", { param0: err instanceof Error ? err.message : String(err) }));
      setDownloadingPack(null);
    }
  };
  ...
  const handleLoad = () => {
    if (confirm(t("读取存档将覆盖当前未保存的进度，确认读取？"))) {
  ...
  const handleRestart = () => {
    if (confirm(t("确认重置文明并重新开始吗？这会抹去当前的纪元进度。"))) {
      GameInstance.reset();
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center bg-[#070B14]/60 p-3 border border-[#243245]/20 rounded">
                    <div>
                      <div>{t("高对比度辅助显示")}</div>
                      <div className="text-[10px] text-[var(--text-secondary)]/50 mt-0.5">{t("提高文本亮度，移除多余虚光滤镜")}</div>
                    </div>
                    <input 
                      type="checkbox" 
            ...
            {activeTab === 'help' && (
              <div className="space-y-3 leading-relaxed overflow-y-auto max-h-72 pr-1">
                ...
                <div className="space-y-2 text-[10px] text-slate-300">
                  <p className="font-bold text-white">{t("【按键操作映射】")}</p>
                  <ul className="list-disc pl-4 space-y-1">
                    <li><kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">{t("空格键")}</kbd>{t(": 推进下一回合 / 抉择事件快速前移")}</li>
                    <li><kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">1</kbd>, <kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">2</kbd>, <kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">3</kbd>{t(": 对应触发抉择事件中的 1-3 选项")}</li>
                    <li><kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">M</kbd>{t(": 星图 (Starmap) |")}<kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">I</kbd>{t(": 情报 (Intel) |")}<kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">T</kbd>{t(": 科技 (Tech)")}</li>
                    <li><kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">G</kbd>{t(": 内阁政府 (Gov) |")}<kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">A</kbd>{t(": 岁月史书 (Archive)")}</li>
                    <li><kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">F</kbd>{t(": 舰队调配指挥中心 |")}<kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded">Esc</kbd>{t(": 关闭当前任何浮层面板")}</li>
                  </ul>
                  <p className="font-bold text-white mt-3">{t("【执政官战略纲领】")}</p>
                  <p>{t("1. 稳定度代表你文明生命线的健康度，低于 30% 时面临极高崩溃风险。")}</p>
                  <p>{t("2. 逃亡倾向过高将诱发社会失控，请妥善通过社会和文化部疏导控制。")}</p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'credits' && (
              <div className="space-y-4 text-center py-6">
                ...
                <div className="space-y-1 text-slate-300 text-[11px] font-mono leading-relaxed">
                  <p>{t("美术定位总监：银河文明全息档案馆")}</p>
                  <p>{t("架构重构团队：Google DeepMind - Antigravity")}</p>
                  <p>{t("编译调试与高阶审美校对：Gemini 3.5 Pro & Flash")}</p>
                  <p className="mt-2 text-[var(--color-primary)]">{t("当前版本：v")}{GAME_VERSION}</p>
            ...
            {activeTab === 'storage' && (
              <div className="space-y-4 flex flex-col justify-between h-full">
                    ...
                    {assetStats.packsDetail.map((pack: any) => (
                      <div key={pack.packId} className="flex justify-between items-center bg-black/35 px-2.5 py-1.5 rounded border border-[#243245]/15 text-[10px]">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {pack.state === 'complete' ? (
                            <span className="text-emerald-400 font-bold">{t("已缓存")}</span>
                          ) : downloadingPack === pack.packId ? (
                            <span className="text-amber-400 font-bold animate-pulse">{t("下载中")}{downloadProgress}%</span>
                          ) : (
                            <button
                ...
                <div className="border-t border-[#243245]/30 pt-3 flex justify-between items-center mt-auto shrink-0">
                  <span className="text-[9px] text-[var(--text-secondary)]/50">{t("发现异常或平衡性问题？")}</span>
                  <button
                  ...
                  >
                    <MessageSquare size={12} className="text-[var(--color-primary)]" /> {t("反馈问题与建议 (GitHub Issues)")}</button>
                </div>
              </div>
            )}
