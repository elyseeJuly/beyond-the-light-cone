        ...
        <div className="flex-1 bg-[#070B14]/40 border border-[#243245]/30 rounded p-5 flex flex-col overflow-y-auto relative">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div className="p-3 bg-[#070B14]/60 border border-[#243245]/20 rounded font-mono">
                    <div className="text-[10px] text-[var(--text-secondary)]">{t("当前总产值 / 经济")}</div>
                    <div className="text-lg font-bold text-white mt-1">{Math.floor(earth.economy)} {t("点")}</div>
                  </div>
                  <div className="p-3 bg-[#070B14]/60 border border-[#243245]/20 rounded font-mono">
                    <div className="text-[10px] text-[var(--text-secondary)]">{t("矿产储备 / 资源")}</div>
                    <div className="text-lg font-bold text-white mt-1">{earth.resource} {t("点")}</div>
                  </div>
                </div>
                <div className="text-xs text-[var(--text-secondary)] leading-relaxed">
                  {t("财政部负责统筹调配行星重工业、太空采矿业以及联合政府的资源供给。如果矿产或经济枯竭，所有在建太空城及太空设施都将面临停滞危险。")}
                </div>
              </div>
          ...
          {activeTab === 'military' && (
            <div className="min-h-full flex flex-col justify-between gap-4">
              <div className="space-y-4">
                <div className="text-sm font-bold text-white border-b border-[#243245]/20 pb-2 flex justify-between items-center">
                  <span>{t("战略司令部与军事投射力量")}</span>
                  <span className="text-xs text-orange-400 font-mono">CODE: MIL-DEPT</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div className="p-3 bg-[#070B14]/60 border border-[#243245]/20 rounded font-mono">
                    <div className="text-[10px] text-[var(--text-secondary)]">{t("地球常备兵力")}</div>
                    <div className="text-lg font-bold text-white mt-1">{earth.army} {t("指数")}</div>
                  </div>
                  <div className="p-3 bg-[#070B14]/60 border border-[#243245]/20 rounded font-mono">
                    <div className="text-[10px] text-[var(--text-secondary)]">{t("现役舰队数量")}</div>
                    <div className="text-lg font-bold text-white mt-1">{earth.fleets.length} {t("支编制")}</div>
                  </div>
                </div>
                <div className="text-xs text-[var(--text-secondary)] leading-relaxed">
                  {t("军事部掌控第一至第五太空防卫编队。在黑暗森林宇宙中，威慑度是保护恒星免遭打击的核心指标。当太空城遭到外星文明入侵时，舰队将自动编入防御作战。")}
                </div>
              </div>
            ...
            <div className="min-h-full flex flex-col justify-between gap-4">
              <div className="space-y-4">
                ...
                <div className="p-3 bg-[#070B14]/60 border border-[#243245]/20 rounded font-mono text-xs text-[var(--text-secondary)] leading-relaxed">
                  {t("目前科技部重点监控三大前沿：天体物理、航天动力学、核技术。通过文化（即科学信息值）积累来冲破智子封锁。")}
                </div>
              </div>
            ...
            <div className="min-h-full flex flex-col justify-between gap-4">
              <div className="space-y-4">
                ...
                <div className="p-3 bg-[#070B14]/60 border border-[#243245]/20 rounded">
                  <div className="text-[10px] text-[var(--text-secondary)] mb-2 font-mono uppercase">{t("当前面壁者阵营：")}</div>
                  {earth.wallfacers.size === 0 ? (
                    <div className="text-xs text-red-400 italic font-mono">-- {t("无活跃面壁者")} --</div>
                  ) : (
                ...
                <div className="text-xs text-[var(--text-secondary)] leading-relaxed">
                  {t("面壁者安全总署负责阻断破壁人。所有面壁者都在面壁者最高委员会的庇护下拥有极高的调兵权，但这也增加了欺骗概率。")}
                </div>
              </div>
