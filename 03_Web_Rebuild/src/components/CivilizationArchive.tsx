...
export const CivilizationArchive: React.FC = () => {
  ...
  const majorEvents = useMemo(() => {
    return playerTimeline.filter(evt => 
      evt.event.includes(t("【文明升级】")) || 
      evt.event.includes(t("【面壁计划】")) || 
      evt.event.includes(t("【破壁人降临】")) || 
      evt.event.includes(t("【数字永生】")) || 
      evt.event.includes(t("【黑暗森林广播】")) || 
      evt.event.includes(t("【高维坍缩】")) || 
      evt.event.includes(t("已完全部署就绪")) ||
      evt.event.includes(t("【纪元更替】")) ||
      evt.event.includes(t("【黑暗森林打击】")) ||
      evt.event.includes(t("【二向箔】")) ||
      evt.event.includes(t("【维度打击】")) ||
      evt.event.includes(t("【威慑】")) ||
      evt.event.includes(t("【征服】")) ||
      evt.event.includes(t("【流浪地球】")) ||
      evt.event.includes(t("【胜利】")) ||
      evt.event.includes(t("【失败】")) ||
      evt.event.includes(t("【结局】"))
    );
  }, [playerTimeline]);
  ...
  const achievements = useMemo(() => {
    const list = [
      { id: 'wallfacer', name: t("面壁壁障"), desc: t("启动旨在欺骗三体的面壁计划"), unlocked: game.hasFlag('wallfacer_project') },
      { id: 'swordholder', name: t("执剑守护者"), desc: t("任命终极核阻断执剑人"), unlocked: game.hasFlag('swordholder_appointed') },
      { id: 'exodus', name: t("掩体避难所"), desc: t("在木星群轨道建立星港城市"), unlocked: game.hasFlag('galaxy_exodus_seen') },
      { id: 'wandering', name: t("光速宣言"), desc: t("实施流浪逃亡计划并达成光速推进"), unlocked: game.hasFlag('wandering_completed') },
      { id: 'digital', name: t("数字永生者"), desc: t("主意识备份并上传至数字方舟中"), unlocked: game.hasFlag('digital_ark_upgrade') },
      { id: 'safety', name: t("安全回声"), desc: t("发布宇宙安全声明，构建星系黑域"), unlocked: game.hasFlag('safety_declaration') || game.hasFlag('black_domain_completed') }
    ];
    return list;
  }, [game]);
  ...
  const getFriendshipLabel = (friendship: number) => {
    const labels = [t("极度敌对"), t("敌对"), t("冷淡"), t("友好"), t("亲密")];
    return labels[friendship] || t("未知");
  };
    ...
    <div className="h-full w-full p-6 flex flex-col overflow-hidden select-none">
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-4 md:gap-6 overflow-hidden">
        ...
        <div className="w-full md:w-48 flex flex-row md:flex-col gap-1.5 shrink-0 overflow-x-auto md:overflow-x-visible pb-2 md:pb-0">
          ...
          >
            <AlertCircle size={14} /> {t("结局预测")}</button>
        ...
        <div className="flex-1 bg-[#070B14]/40 border border-[#243245]/30 rounded p-5 flex flex-col overflow-hidden relative">
          ...
          {activeTab === 'major_events' && (
            <div className="h-full flex flex-col overflow-hidden">
              <div className="text-[10px] font-title font-bold text-[var(--color-primary)] mb-3 shrink-0 uppercase tracking-wider">
                {t("文明史重大碑石记录 (当前共")}{majorEvents.length} {t("条)")}</div>
              <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                {majorEvents.length === 0 ? (
                  <div className="text-center py-12 text-xs text-[var(--text-secondary)] italic">{t("暂无列入银河档案馆的重大文明事件。")}</div>
                ) : (
                  [...majorEvents].reverse().map((evt, idx) => (
                    <div key={idx} className="p-3 bg-[#070B14]/80 border-l-2 border-red-500 rounded-r border-y border-r border-[#243245]/20 font-mono text-xs space-y-1">
                      <div className="text-[var(--color-primary)] font-bold">{t("纪元历史档案 第")}{evt.year} {t("年")}</div>
          ...
          {activeTab === 'unlocked_techs' && (
            <div className="h-full flex flex-col overflow-hidden">
              <div className="text-[10px] font-title font-bold text-[var(--color-primary)] mb-3 shrink-0 uppercase tracking-wider">
                {t("已解码并投入使用的科学技术 (共")}{finishedTechs.length} {t("项)")}</div>
              <div className="flex-1 overflow-y-auto pr-1">
                {finishedTechs.length === 0 ? (
                  <div className="text-center py-12 text-xs text-[var(--text-secondary)] italic">{t("基础物理被锁死中，尚无已解锁的高级科技。")}</div>
                ) : (
            ...
            <div className="h-full flex flex-col overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                {discoveredCivs.length === 0 ? (
                  <div className="text-center py-12 text-xs text-[var(--text-secondary)] italic">{t("全频段宁静。尚未发现任何地外文明踪迹。")}</div>
                ) : (
                  discoveredCivs.map((civ, idx) => (
                    <div key={idx} className="p-3.5 bg-[#070B14]/60 border border-[#243245]/30 rounded flex justify-between items-center font-mono text-xs">
                      <div>
                        <div className="font-bold text-white text-sm">{civ.name} {t("文明")}</div>
                        <div className="text-[10px] text-[var(--text-secondary)] mt-1">
                          {t("常驻人口:")}{civ.population} {t("点 | 外交友好度:")}{getFriendshipLabel(civ.friendship)}
                          {!civ.contacted && <span className="ml-2 text-amber-400">{t("[未建立通信]")}</span>}
                        </div>
                      </div>
